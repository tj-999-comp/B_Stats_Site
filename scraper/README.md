...
GitHub Actionsの`Daily B.League game scrape`は毎日03:00 JSTに前日分を取得します。現在はB1（`tab=1`）だけを対象にし、手動実行では`target_date`、`season`、`max_retries`を指定できます。WorkflowはDBを変更せず、取得JSONとログをArtifactへ保存します。B2・B.LEAGUE NEXTは現行DBの対応範囲外です。
