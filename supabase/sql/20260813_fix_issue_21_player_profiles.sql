-- Issue #21: apply manually reviewed player profile patch
-- Date: 2026-08-13
-- Related issues: #12, #21
-- Source: supabase/patches/20260813_issue21_missing_player_profiles.csv
-- Scope: 117 players; staff-like and dummy IDs excluded by Issue #12 are absent.
-- Run order: backup first, then verify before apply, then this file, then verify after apply.
-- Policy: blank CSV cells are ignored; existing nonblank DB values are not overwritten.
-- Re-run guard: the target rows must still match the backup snapshot and at least one fill must remain.
...
DECLARE
    ...
    patch_rows INTEGER;
    live_rows INTEGER;
    backup_mismatches INTEGER;
    ready_rows INTEGER;
    updated_rows INTEGER;
    remaining_mismatches INTEGER;
    updated_at_unchanged INTEGER;
BEGIN
    ...
    SELECT COUNT(*) INTO patch_rows FROM issue_21_profile_patch;
    IF patch_rows <> 117 THEN
        RAISE EXCEPTION 'patch row count mismatch: expected=117 actual=%', patch_rows;
    END IF;

    SELECT COUNT(*) INTO live_rows
      FROM public.players p
      JOIN public.data_patch_backup_20260813_issue_21_player_profiles b USING (player_id);
    IF live_rows <> 117 THEN
        RAISE EXCEPTION 'live target row count mismatch: expected=117 actual=%', live_rows;
    END IF;
    ...
    SELECT COUNT(*) INTO ready_rows
     ...
     WHERE (
       (patch.player_name_j IS NOT NULL
        AND NULLIF(p.player_name_j, '') IS NULL)
    OR (patch.player_name_e IS NOT NULL
        AND NULLIF(p.player_name_e, '') IS NULL)
    OR (patch.last_seen_jersey_number IS NOT NULL
        AND NULLIF(p.last_seen_jersey_number, '') IS NULL)
    OR (patch.last_seen_team_id IS NOT NULL
        AND NULLIF(p.last_seen_team_id, '') IS NULL)
    OR (patch.league_registered_nationality IS NOT NULL
        AND NULLIF(p.league_registered_nationality, '') IS NULL)
    OR (patch.birthplace IS NOT NULL
        AND NULLIF(p.birthplace, '') IS NULL)
);
    IF ready_rows = 0 THEN
        RAISE EXCEPTION 'no Issue #21 profile fields remain to fill; fix may already be applied';
    END IF;

    UPDATE public.players AS p
       SET player_name_j = CASE
               WHEN patch.player_name_j IS NOT NULL
                AND NULLIF(p.player_name_j, '') IS NULL
                   THEN patch.player_name_j
               ELSE p.player_name_j
           END,
           player_name_e = CASE
               WHEN patch.player_name_e IS NOT NULL
                AND NULLIF(p.player_name_e, '') IS NULL
                   THEN patch.player_name_e
               ELSE p.player_name_e
           END,
           last_seen_jersey_number = CASE
               WHEN patch.last_seen_jersey_number IS NOT NULL
                AND NULLIF(p.last_seen_jersey_number, '') IS NULL
                   THEN patch.last_seen_jersey_number
               ELSE p.last_seen_jersey_number
           END,
           last_seen_team_id = CASE
               WHEN patch.last_seen_team_id IS NOT NULL
                AND NULLIF(p.last_seen_team_id, '') IS NULL
                   THEN patch.last_seen_team_id
               ELSE p.last_seen_team_id
           END,
           league_registered_nationality = CASE
               WHEN patch.league_registered_nationality IS NOT NULL
                AND NULLIF(p.league_registered_nationality, '') IS NULL
                   THEN patch.league_registered_nationality
               ELSE p.league_registered_nationality
           END,
           birthplace = CASE
               WHEN patch.birthplace IS NOT NULL
                AND NULLIF(p.birthplace, '') IS NULL
                   THEN patch.birthplace
               ELSE p.birthplace
           END,
           updated_at = NOW()
      FROM issue_21_profile_patch AS patch
    ...
    IF updated_rows <> ready_rows THEN
        RAISE EXCEPTION 'updated row count mismatch: expected=% actual=%',
            ready_rows, updated_rows;
    END IF;
    ...
    IF remaining_mismatches <> 0 THEN
        RAISE EXCEPTION 'post-apply profile mismatch: % rows', remaining_mismatches;
    END IF;

    SELECT COUNT(*) INTO updated_at_unchanged
      FROM public.players p
      JOIN public.data_patch_backup_20260813_issue_21_player_profiles b USING (player_id)
      JOIN issue_21_profile_patch patch USING (player_id)
     WHERE (
    ...
    IF updated_at_unchanged <> 0 THEN
        RAISE EXCEPTION 'updated_at was not changed for % updated rows', updated_at_unchanged;
    END IF;
END;
$issue_21_apply$;
