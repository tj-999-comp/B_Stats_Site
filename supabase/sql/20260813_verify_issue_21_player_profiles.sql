-- Issue #21: verify before/after/rollback state
-- Date: 2026-08-13
-- Related issues: #12, #21
-- Backup: public.data_patch_backup_20260813_issue_21_player_profiles
-- This file is read-only against live tables; it may create a session-local temp table.
...
BEGIN
    IF TO_REGCLASS('public.data_patch_backup_20260813_issue_21_player_profiles') IS NULL THEN
        RAISE EXCEPTION 'Issue #21 backup table is required; run backup SQL first';
    END IF;
...
SELECT 'change_plan' AS check_group, 'players.target_rows' AS item, COUNT(*)::BIGINT AS row_count
  FROM public.players p
  JOIN public.data_patch_backup_20260813_issue_21_player_profiles b USING (player_id)
UNION ALL
SELECT 'change_plan', 'players.player_name_j_fillable', COUNT(*)
  FROM public.data_patch_backup_20260813_issue_21_player_profiles b
  JOIN issue_21_profile_patch patch USING (player_id)
 WHERE NULLIF(b.player_name_j, '') IS NULL
   AND patch.player_name_j IS NOT NULL
UNION ALL
SELECT 'change_plan', 'players.player_name_e_fillable', COUNT(*)
  FROM public.data_patch_backup_20260813_issue_21_player_profiles b
  JOIN issue_21_profile_patch patch USING (player_id)
 WHERE NULLIF(b.player_name_e, '') IS NULL
   AND patch.player_name_e IS NOT NULL
UNION ALL
SELECT 'change_plan', 'players.last_seen_jersey_number_fillable', COUNT(*)
  FROM public.data_patch_backup_20260813_issue_21_player_profiles b
  JOIN issue_21_profile_patch patch USING (player_id)
 WHERE NULLIF(b.last_seen_jersey_number, '') IS NULL
   AND patch.last_seen_jersey_number IS NOT NULL
UNION ALL
SELECT 'change_plan', 'players.last_seen_team_id_fillable', COUNT(*)
  FROM public.data_patch_backup_20260813_issue_21_player_profiles b
  JOIN issue_21_profile_patch patch USING (player_id)
 WHERE NULLIF(b.last_seen_team_id, '') IS NULL
   AND patch.last_seen_team_id IS NOT NULL
UNION ALL
SELECT 'change_plan', 'players.league_registered_nationality_fillable', COUNT(*)
  FROM public.data_patch_backup_20260813_issue_21_player_profiles b
  JOIN issue_21_profile_patch patch USING (player_id)
 WHERE NULLIF(b.league_registered_nationality, '') IS NULL
   AND patch.league_registered_nationality IS NOT NULL
UNION ALL
SELECT 'change_plan', 'players.birthplace_fillable', COUNT(*)
  FROM public.data_patch_backup_20260813_issue_21_player_profiles b
  JOIN issue_21_profile_patch patch USING (player_id)
 WHERE NULLIF(b.birthplace, '') IS NULL
   AND patch.birthplace IS NOT NULL
UNION ALL
SELECT 'accepted_source_blank', 'last_seen_team_id', COUNT(*)
  FROM issue_21_profile_patch
 WHERE last_seen_team_id IS NULL
UNION ALL
SELECT 'accepted_source_blank', 'league_registered_nationality', COUNT(*)
  FROM issue_21_profile_patch
 WHERE league_registered_nationality IS NULL
UNION ALL
SELECT 'accepted_source_blank', 'birthplace', COUNT(*)
  FROM issue_21_profile_patch
...
WITH compared AS (
    SELECT
        ...
        (
            p.player_name_j IS DISTINCT FROM b.player_name_j
        ...
        (
            p.player_name_j IS DISTINCT FROM
...
expected AS (
    SELECT COUNT(*)::BIGINT AS expected_changed_rows
      FROM public.data_patch_backup_20260813_issue_21_player_profiles b
      JOIN issue_21_profile_patch patch USING (player_id)
     WHERE (
...
summary AS (
    SELECT
        ...
        COUNT(*) FILTER (WHERE before_mismatch)::BIGINT AS before_mismatches,
        COUNT(*) FILTER (WHERE after_mismatch)::BIGINT AS after_mismatches,
        COUNT(*) FILTER (WHERE profile_changed)::BIGINT AS current_profile_changed_rows,
        COUNT(*) FILTER (
            WHERE current_updated_at IS DISTINCT FROM backup_updated_at
        )::BIGINT AS current_updated_at_changed_rows
    FROM compared
)
SELECT
    CASE
        WHEN profile_rows <> 117 THEN 'UNEXPECTED_STATE'
        WHEN before_mismatches = 0
         AND current_updated_at_changed_rows = 0
            THEN 'BEFORE_APPLY_OR_AFTER_ROLLBACK'
        WHEN after_mismatches = 0
         AND current_profile_changed_rows = expected_changed_rows
         AND current_updated_at_changed_rows = expected_changed_rows
         AND expected_changed_rows > 0
            THEN 'AFTER_APPLY'
    ...
    current_profile_changed_rows,
    current_updated_at_changed_rows
  FROM summary
  CROSS JOIN expected;
...
SELECT
    ...
    p.player_name_j,
    p.player_name_e,
    p.last_seen_jersey_number,
    p.last_seen_team_id,
    p.league_registered_nationality,
    p.birthplace
  FROM public.players p
  JOIN public.data_patch_backup_20260813_issue_21_player_profiles b USING (player_id)
  JOIN issue_21_profile_patch patch USING (player_id)
