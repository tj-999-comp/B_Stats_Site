#!/usr/bin/env python3
"""Validate the Markdown and metadata contract for A-rendered work records.

HTML is generated by sandbox-pages and is intentionally not read or generated
here. This validator intentionally uses only the Python standard library so
the same checks can run locally and in GitHub Actions without installing extra
packages.
"""
...
def run_fixtures() -> int:
    """Exercise the Markdown/metadata checks without requiring HTML output."""
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if not any("title does not match" in violation for violation in violations):
            print("fixture metadata mismatch was not rejected")
            return 1
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate work-record Markdown and metadata.")
